...
class Command(BaseCommand):
    ...
    def create_camp_info_items(self, camp, categories):
        ...
        InfoItem.objects.create(
            ...
            headline='Opening',
            anchor='opening',
            body='BornHack {} starts saturday, august 27th, at noon (12:00). It will be possible to access the venue before noon if for example you arrive early in the morning with the ferry. But please dont expect everything to be ready before noon :)'.format(year)
        )
        InfoItem.objects.create(
            category=categories['when'],
            headline='Closing',
            anchor='closing',
            body='BornHack {} ends saturday, september 3rd, at noon (12:00). Rented village tents must be empty and cleaned at this time, ready to take down. Participants must leave the site no later than 17:00 on the closing day (or stay and help us clean up).'.format(year)
        )
        InfoItem.objects.create(
    ...
    def create_camp_cfp(self, camp):
        ...
        camp.call_for_participation='Please give a talk at Bornhack {}...'.format(year)

    def create_camp_cfs(self, camp):
        year = camp.camp.lower.year
        self.output("Creating CFS for {}...".format(year))
        camp.call_for_sponsors_open=True
        camp.call_for_sponsors='Please give us ALL the money so that we can make Bornhack {} the best ever!'.format(year)
    ...
    def handle(self, *args, **options):
        ...
        for (camp, read_only) in camps:
            ...
            self.output(self.style.SUCCESS("----------[ Bornhack {} ]----------".format(year)))

            if year < 2020:
                ticket_types = self.create_camp_ticket_types(camp)

                camp_products = self.create_camp_products(camp, product_categories, ticket_types)
