...
import Date.Extra
import Html exposing (Html, a, div, h4, i, li, p, span, table, text, ul)
import Html.Attributes exposing (alt, classList, href, style, title)
import List.Extra
import Messages exposing (Msg(..))
import Models exposing (Day, EventInstance, FilterType(..), Model, Route(..), getNameFromFilterType, getSlugFromFilterType)
...
locationColumn columnWidth eventInstances offset minutes location =
    ...
    in
    div
        [ style "width" (String.fromInt columnWidth ++ "%")
        , classList
...
eventInstanceBlock offset numberInGroup ( eventInstance, lefts ) =
    let
        ...
        height =
            String.fromInt (length * toFloat blockHeight) ++ "px"
    ...
    a
        ...
        , style "left" (String.fromFloat (toFloat lefts * width) ++ "%")
